...
export default function FaithTeaser() {
  return (
    <section className="relative px-6 py-12 md:py-20 bg-gradient-to-br from-sky-50 to-white overflow-hidden">
      {/* Decorative floating elements — only on desktop */}
      <motion.div 
        className="absolute top-1/4 left-1/4 w-16 h-16 bg-red-500/50 rounded-full blur-2xl hidden lg:block"
        animate={{
          y: [-20, 20, -20],
          scale: [1, 1.05, 1]
        }}
      ...
      {/* FLEX CONTAINER for side-by-side (desktop) or stacked (mobile) */}
      <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12 max-w-5xl mx-auto">
        
        {/* LEFT SIDE: Content (appears second on mobile, first on desktop) */}
        <motion.div
          initial={{ opacity: 0, x: -30 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true, margin: "-100px" }}
          transition={{ duration: 0.8, delay: 0.4 }}
          className="lg:flex-1 text-center md:text-left space-y-6 max-w-md mx-auto lg:mx-0"
        >
          {/* Badge */}
          <motion.div
        ...
        <motion.div
          ...
          className="w-full lg:w-[50%] max-w-md mx-auto lg:mx-0"
        >
          <div className="relative h-[300px] sm:h-[350px] md:h-[400px] lg:h-[500px] rounded-3xl shadow-2xl overflow-hidden">
